...
async function OGImageFetcher({ imageUrl, className }: { imageUrl?: string | null; className?: string }) {
  if (!imageUrl) {
    return (
      <div className={cn("w-full flex items-center justify-center bg-muted text-muted-foreground rounded-2xl border aspect-[1.91/1]", className)}>
  ...
  return (
    <div className={cn("w-full relative overflow-hidden rounded-2xl border bg-muted aspect-[1.91/1]", className)}>
...
export const OGPreviewServer: React.FC<OGPreviewServerProps> = ({ imageUrl, className }) => {
  return (
    <Suspense 
      fallback={
        <Skeleton className={cn("w-full rounded-2xl border aspect-[1.91/1]", className)}/>
      }
    >
